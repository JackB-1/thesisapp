...
export default class MovesenseBT extends Component {
  ...
  scanAndConnect() {
    try {
      ...
      this.manager.startDeviceScan(null, null, (error, device) => {
        ...
        if (!found) {
          console.info('Error in scanAndConnect: NOPT FOUND ');
  ...
  async connectToDevice(item) {
    try {
      console.log(`Attempting to connect to device: ${item.name} with ID: ${item.id}`);
      ...
      this.info(`Connecting to ${item.name}...`);
      const connection = await this.manager.connectToDevice(item.id);
      console.log(`Connected to ${item.name}`, connection);
  
      this.deviceList = [];
      console.log('Device list cleared.');
  
      const device = await this.manager.discoverAllServicesAndCharacteristicsForDevice(item.id);
      console.log('Discovered all services and characteristics for device:', device);
  
      const services = await device.services();
      console.log('Services:', services);
  
      const charPromises = services.map(service => service.characteristics());
      console.log('Characteristics promises:', charPromises);
  
      const characteristics = await Promise.all(
        charPromises.map(p => p.catch(e => {
          console.error('Error in characteristics promise:', e);
          return e;
        })),
      );
      console.log('Characteristics:', characteristics);
  
      const service = '34802252-7185-4d5d-b431-630e7050e8f0';
      const characteristicW = '34800001-7185-4d5d-b431-630e7050e8f0';
      const characteristicN = '34800002-7185-4d5d-b431-630e7050e8f0';
      ...
      console.log('Write characteristic response:', characteristic);
      ...
      this.subscriptionMonitor = device.monitorCharacteristicForService(
        ...
        (error, characteristic) => {
          ...
          console.log('Received characteristic update:', characteristic);
